...
class IEMidi:
    def __init__(self, audioPath: str="./data/audio", midiPath: str = "./data/midi", \
                 ...
                 ONSET_OFFSET_FLAG: bool=False, EXTEND_FLAG: bool=False, REF_THRESH: float=0.85,\
        ...
        """
            ...
            Args:
                ...
                EXTEND_FLAG (bool): If true, extend the offsets for calculating scores
            ...
            Returns:
                None
        """
        self.audio_files = sorted(Path(audioPath).glob("*.wav"))[:100]
        self.midi_files = sorted(Path(midiPath).glob("*.mid"))[:100]
        self.midi_score_files = sorted(Path(midiScorePath).glob("*.mid"))[:100]
        self.xml_score_files = sorted(Path(xmlScorePath).glob("*.musicxml"))[:100]
        ...
        self.STORE_PATH = "/home/nkcemeka/Documents/ismir2026/scoreeval/data/interesting_pm"
        ...
        if filtered_idx.size > 0:
            ...
            for each in range(len(filtered_idx)):
                ...
                for model_name in temp_dict.keys():
                    ...
                    xml_beyer_path = str(Path(self.STORE_PATH) / f"pred_xml/{file_stem}_{model_name}_beyer.musicxml")
                    mscore_beyer_path = str(Path(self.STORE_PATH)/ f"pred_mscore/{file_stem}_{model_name}_beyer.mid")
                    mscore_nak_path = str(Path(self.STORE_PATH) / f"pred_mscore/{file_stem}_{model_name}_nak.mid")
                    mscore_pm2s_path = str(Path(self.STORE_PATH) / f"pred_mscore/{file_stem}_{model_name}_pm2s.mid")
                    #xml_path = str(midi_path).replace("pred_midi", "pred_xml").replace("mid", "musicxml")
                    #mscore_path = str(midi_path).replace("pred_midi", "pred_mscore")
                    
                    # If this fails for some reason, skip (something is wrong: e.g.very short piece)
                    try:      
                        # convert into xml with beyer for now (we include others later)
                        beyer_midi_xml(midi_path, xml_beyer_path)

                        # convert beyer's xml into predicted music scores
                        xml_to_midi(xml_beyer_path, mscore_beyer_path)
                        assert Path(mscore_beyer_path).exists()
                        
                        # nakamura & PM2S
                        nakamura_inference(midi_path, mscore_nak_path)
                        pm2s_inference(midi_path, mscore_pm2s_path)

                        assert Path(mscore_nak_path).exists()
                        assert Path(mscore_pm2s_path).exists()
                    except:
                        valid = False

                        # if anything fails, remove the clutter
                        if os.path.exists(xml_beyer_path):
                            os.remove(xml_beyer_path)
                        
                        if os.path.exists(mscore_beyer_path):
                            os.remove(mscore_beyer_path)

                        if os.path.exists(mscore_nak_path):
                            os.remove(mscore_nak_path)
                        
                        if os.path.exists(mscore_pm2s_path):
                            os.remove(mscore_pm2s_path)
                        continue

                    # convert nakamura & PM2S to musicxml with musescore
                    midi_to_xml(mscore_nak_path, mscore_nak_path.replace("mid", "musicxml").replace("pred_mscore", "pred_xml"))
                    midi_to_xml(mscore_pm2s_path, mscore_pm2s_path.replace("mid", "musicxml").replace("pred_mscore", "pred_xml"))

                    # dict to store info for this interesting segment
                    int_dict[f'{model_name}_beyer_xml'] = xml_beyer_path
                    int_dict[f'{model_name}_beyer_mscore'] = mscore_beyer_path
                    int_dict[f'{model_name}_pm2s_xml'] = mscore_pm2s_path.replace("mid", "musicxml").replace("pred_mscore", "pred_xml")
                    int_dict[f'{model_name}_pm2s_mscore'] = mscore_pm2s_path
                    int_dict[f'{model_name}_nak_xml'] = mscore_nak_path.replace("mid", "musicxml").replace("pred_mscore", "pred_xml")
                    int_dict[f'{model_name}_nak_mscore'] = mscore_nak_path
                    # int_dict[f'{model_name}_xml'] = xml_path
                    # int_dict[f'{model_name}_mscore'] = mscore_path
    ...
    def compute_f12(self, midi1: pretty_midi.PrettyMIDI, midi2: pretty_midi.PrettyMIDI) -> float:
        ...
        if self.ONSET_OFFSET_FLAG:
            ...
            _, f12 = get_scores_trans(midi1, midi2) 
            _, f12_check = get_scores_trans(midi2, midi1)
        else:
            f12, _ = get_scores_trans(midi1, midi2)
            f12_check, _ = get_scores_trans(midi2, midi1)
...
ie = IEMidi(ONSET_OFFSET_FLAG=True, REF_SIM_THRESH=0.07, REF_THRESH=0.9)
ie.gen_interesting()
