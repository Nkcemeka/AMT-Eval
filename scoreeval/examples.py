...
class ExScore:
    ...
    def sample(self, xlsx_file: str):
        ...
        sel_samples = random.sample(list(final_hash.keys()), 10)
        final_selection = []
...
class ExMidi:
    ...
    def gen_examples(self):
        ...
        for idx, (audio_file, midi_file, midi_score_file, xml_score_file) in tqdm(enumerate(zip(self.audio_files, \
            ...
            try:
                ...
                print(f"F12: {f12}", f"1-F12: {1-f12}", f"FAB: {fab}",  audio_file, arr_scores[top1_index], arr_scores[top2_index])
            except:
                continue
        ...
        if fab_list and f12_list:
            ...
            if np.sum(final_mask) > 0:
                negate_mask = np.logical_not(final_mask).astype(bool)
                print(np.sum(final_mask), np.sum(negate_mask))
                filtered_idx = np.array(list(idx_dict.keys()))[final_mask]

                filt_fab = fab_arr[final_mask]
                filt_diff = one_minus_f12_arr[final_mask]
                rest_fab = fab_arr[negate_mask] # rest of items not selected
                rest_diff = one_minus_f12_arr[negate_mask]

                print(rest_fab.shape, len(rest_fab))

                # Randomly select 10% of items not selected to be part of the dataset
                sel_indices = np.random.choice(len(rest_fab), size=int(0.1*len(rest_fab)), replace=False)
                mask_rand_sel =  np.zeros(rest_fab.shape).astype(bool)
                mask_rand_sel[sel_indices] = 1
                sel_fab, sel_diff = rest_fab[mask_rand_sel], rest_diff[mask_rand_sel]
                sel_idx = np.array(list(idx_dict.keys()))[negate_mask][mask_rand_sel] # we want to add this to dataset as well
                print(len(sel_fab), len(sel_diff), sel_indices.shape, sel_indices)
                others_fab, others_diff = rest_fab[np.logical_not(mask_rand_sel).astype(bool)],\
                    rest_diff[np.logical_not(mask_rand_sel).astype(bool)]

                # so we have selection, others and filtered
                # which is what we want
                # Make a scatter plot
                upperLeft, bottomRight, w, h = self.get_rect(filt_fab, filt_diff)
                rect = patches.Rectangle((upperLeft[0], bottomRight[1]), w, h, linewidth=1, edgecolor='g', facecolor='none')
                plt.scatter(filt_fab, np.abs(filt_diff), color='red')
                plt.scatter(others_fab, np.abs(others_diff), color='blue')
                plt.scatter(sel_fab, np.abs(sel_diff), color='red')
                plt.xlabel("|Fr1 -Fr2|")
                plt.ylabel("|1 - F12|")
                plt.xlim(0, np.max(fab_arr)+0.05)
                plt.ylim(bottom=0)
                plt.title('Scatter plot of |1 - F12| against |Fr1 -Fr2|')
                ...
                # Randomly select 10% of items not selected to be part of the dataset
                sel_indices = np.random.choice(len(fab_arr), size=int(0.1*len(fab_arr)), replace=False)
                mask_rand_sel =  np.zeros(fab_arr.shape).astype(bool)
                mask_rand_sel[sel_indices] = 1
                sel_fab, sel_diff = fab_arr[mask_rand_sel], one_minus_f12_arr[mask_rand_sel]
                sel_idx = np.array(list(idx_dict.keys()))[mask_rand_sel] # we want to add this to dataset as well
                others_fab, others_diff = fab_arr[np.logical_not(mask_rand_sel).astype(bool)],\
                    one_minus_f12_arr[np.logical_not(mask_rand_sel).astype(bool)]
                # Make a scatter plot
                plt.scatter(others_fab, np.abs(others_diff), color='blue')
                plt.scatter(sel_fab, np.abs(sel_diff), color='red')
                plt.xlabel("|Fr1 -Fr2|")
        ...
        # all the indices or points we have selected to be a part of our dataset
        dataset_idx = [filtered_idx, sel_idx] 
        
        for d_idx in dataset_idx:
            if d_idx.size > 0:
                # Save these interesting files
                for each in range(len(d_idx)):
                    audio_file = self.audio_files[d_idx[each].item()]
                    midi_file = self.midi_files[d_idx[each].item()]
                    midi_score_file = self.midi_score_files[d_idx[each].item()]
                    xml_score_file = self.xml_score_files[d_idx[each].item()]
                    ...
                    # I don't know if this is redundant but will look into it later
        ...
        if interesting_files:
            ...
            # Generate the audio for the transcriptions
            self.gen_audio(f"{str(self.STORE_PATH)}/examples.json")
            self.gen_sproll(f"{str(self.STORE_PATH)}/examples.json")
    ...
    def get_rect(self, arr_x: np.ndarray, arr_y: np.ndarray) -> tuple:
        sorted_arr_y = np.sort(arr_y)
        sorted_arr_x = np.sort(arr_x)
        max_y = sorted_arr_y[-1]
        max_x = sorted_arr_x[-1]
        upperLeft = (0, max_y+0.03)
        bottomRight = (max_x, self.MODEL_SIM_THRESH)
        width = max_x
        height = max_y + 0.03 - self.MODEL_SIM_THRESH
        return upperLeft, bottomRight, width, height
    
    def gen_sproll(self, json_file: str):
        """
            Generate sprolls (spectrogram-piano rolls)
            for every transcription!

            Args:
                json_file (str): Path to json file containing examples
        """
        #AUDIO_STORE_DIR = Path(self.STORE_PATH) / "pred_audio"
        AUDIO_STORE_DIR = Path("/home/nkcemeka/Documents/ismir2026/scoreeval/data/audio")
        # pred_audio_files = AUDIO_STORE_DIR.glob("*.wav")
        with open(json_file, "r") as f:
            exs = json.load(f)

        for each in tqdm(exs["files"]):
            min_pitch = each["min_pitch"]
            max_pitch = each["max_pitch"]
            for k in each.keys():
                if "_pmidi" in k:
                    midi_path = Path(each[k])
                    midi_name = midi_path.stem
                    audio_ref_name = "_".join(midi_name.split("_")[:-1])
                    #print(midi_name, audio_ref_name)
                    #exit(1)
                    audio_path = AUDIO_STORE_DIR / f"{audio_ref_name}.wav"
                    midi_obj = pretty_midi.PrettyMIDI(str(midi_path))
...
# a = ExMidiBasic(model1='hft', model2='oaf')
# a.sample("./data/midi_examples/examples.json")
#a.gen_examples()

# a = ExHarm()
# a.gen_examples()

# a = ExMidi(REF_THRESH=0)
# a.gen_sproll("./data/midi_examples/examples.json")
#a.gen_examples()
# a.gen_audio("./data/midi_examples/examples.json")

a = ExScore()
a.sample("data/score_examples/output.xlsx")
#a.gen_examples()
#a.gen_plots('musescore', 'beyer')
# a.gen_audio("./data/score_examples/examples.json")
